...
uniform sampler2D   Rd; // @0 'Surface diffuse reflectance texture'
uniform sampler2D   Rs; // @1 'Surface specular reflectance texture'
uniform sampler2D   Nm; // @0 'Surface normal texture'
uniform sampler2D   As; // @1 'Surface ambient specular texture'

// @cb0
uniform lowp vec3   Cs;      // constant 'Specular color'
uniform lowp vec3   Cvn;     // constant 'Fresnel color'
uniform lowp mat3   LcolorA; // 'Light color (array)'
uniform float       r;       // constant 'Surface roughness'


varying vec2        T2d;     // 'Surface texture coordinate'
varying vec3        N0w;     // 'Normal direction, before normalization and bump'
varying vec3        Tw;      // 'Tangent direction'
varying vec3        L0w[Li]; // 'Light direction, before normalization'
varying vec3        V0w;     // 'View direction, before normalization'
...
void main( void )
{
	...
	vec3  Ns        = texture2D(Nm, T2d).xyz * 2.0 - 1.0;            // sample_normal
	...
	lowp vec3    Casc = texture2D(As, R.xy).xyz;  // specular_ambient_factor
	lowp vec3    Crd  = texture2D  (Rd, T2d).xyz; // diffuse_surface_factor
	lowp vec3    Crs  = texture2D  (Rs, T2d).xyz; // specular_surface_factor
	...
	gl_FragColor = vec4( (Cdd * Crd) + CrsSvn*(Casc+ECSH * Csd), 1.0 );
}
